...
\usepackage{amsmath}
\usepackage{textcomp}
\usepackage{amssymb}       % blackboard / empty-set symbols (e.g. \varnothing)
% Chinese text support. Fonts are chosen from the ones actually installed on
% the machine, so the template works on macOS, Windows and Linux alike.
% This template targets XeLaTeX / LuaLaTeX; pdflatex cannot embed CJK fonts.
...
% Pandoc syntax highlighting (defines Shaded/Highlighting environments and token commands)
$highlighting-macros$
$endif$
\title{$title$}
...
$else$
\setcounter{secnumdepth}{-\maxdimen} % suppress auto numbering: notes carry their own
$endif$
